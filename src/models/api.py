...
class IngestSummary(BaseModel):
    ...
    total_received: int = Field(..., description="Total number of items received in the request")
    accepted: int = Field(..., description="Number of items that passed validation and filtering")
    rejected: int = Field(..., description="Number of items that failed relevance filtering")
    duplicates: int = Field(..., description="Number of duplicate items (already exist in storage)")
    errors: int = Field(..., description="Number of items that failed validation or processing")
...
class IngestResponse(BaseModel):
    ...
    summary: IngestSummary = Field(..., description="Detailed statistics of the ingestion process")
...
class HealthResponse(BaseModel):
    ...
    dependencies: Dict[Literal["storage", "filters", "sources"], DependencyHealth] = Field(
        ...,
...
class ValidationErrorResponse(BaseModel):
    ...
    details: Optional[List[Dict]] = Field(
        default=None,
        description=(
            "Detailed error information, especially for validation errors"
        )
    )
...
class BadRequestErrorResponse(BaseModel):
    ...
    details: Optional[List[Dict]] = Field(
        default=None,
        description="Error details for bad request"
    )
...
class InternalServerErrorResponse(BaseModel):
    ...
    details: Optional[List[Dict]] = Field(
        default=None,
        description="Error details for internal server error"
    )
...
class RootResponse(BaseModel):
    ...
    description: str = Field(..., description="Detailed description of the API functionality")
    ...
    endpoints: Optional[Dict[str, str]] = Field(
        default=None,
        description="Available API endpoints and their paths"
    )
